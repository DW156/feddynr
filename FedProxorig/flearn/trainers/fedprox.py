    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            if i % self.eval_every == 0:
                ...
                tqdm.write('At round {} accuracy: {:.2f}'.format(i, np.sum(stats[3])*1.0/np.sum(stats[2])))  # testing accuracy
                tqdm.write('At round {} training accuracy: {:.2f}'.format(i, np.sum(stats_train[3])*1.0/np.sum(stats_train[2])))
                tqdm.write('At round {} training loss: {:.2f}'.format(i, np.dot(stats_train[4], stats_train[2])*1.0/np.sum(stats_train[2])))
            ...
            tqdm.write('gradient difference: {:.2f}'.format(difference))
            ...
            for idx, c in enumerate(selected_clients.tolist()):
                ...
                c.load_state_dict(self.latest_model)
        ...
        tqdm.write('At round {} accuracy: {:.2f}'.format(self.num_rounds, np.sum(stats[3])*1.0/np.sum(stats[2])))
        tqdm.write('At round {} training accuracy: {:.2f}'.format(self.num_rounds, np.sum(stats_train[3])*1.0/np.sum(stats_train[2])))
